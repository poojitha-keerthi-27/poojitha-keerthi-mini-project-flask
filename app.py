...
Session(app)
mydb=mysql.connector.connect(host='localhost',user='root',password='Poojitha@777',db='spm')
app.secret_key=b'S\x91b\xe6\xc4'
@app.route('/')
def index():
...
def verifyotp(data1):
    try:
        ...
        if request.method=='POST':
            ...
            if uotp==data1['otp']:
                cursor=mydb.cursor(buffered=True)
                #cursor.execute('insert into student1(email,student_fname,student_lname,password) values(%s,%s,%s,%s)',[email,stu_fname,stu_lname,password])
                cursor.execute('insert into student1(email,student_fname,student_lname,password) values(%s,%s,%s,%s)',[data1['email'],data1['stu_fname'],data1['stu_lname'],data1['password']])
                mydb.commit()
                cursor.close()
                print('d')
...
def login():
    if session.get('email'):
        ...
        if request.method=='POST':
            ...
            try:
                cursor=mydb.cursor(buffered=True)
                cursor.execute('select email,password from student1 where email=%s',[email])
...
def updatenotes(n_id):
    if not session.get('email'):
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select title,note_content from notes where n_id=%s',[n_id])
        note_data=cursor.fetchone()
        if request.method=='POST':
            title=request.form['title']
            content=request.form['content']
            #cursor=mydb.cursor(buffered=True)
            cursor.execute('update notes set title=%s,note_content=%s where n_id=%s',[title,content,n_id])
            mydb.commit()
            cursor.close()
            flash(f'notes with {title} updated successfully')
            return redirect(url_for('updatenotes',n_id=n_id))
...
def deletenotes(n_id):
    if not session.get('email'):
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('delete from notes where n_id=%s and added_by=%s',[n_id,session.get('email')])
        mydb.commit()
        cursor.close()
        flash(f'notes {n_id} deleted successfully')
        return redirect(url_for('panel'))
@app.route('/allnotes')
def allnotes():
    if not session.get('email'):
        ...
        added_by=session.get('email')
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select n_id,title,created_at from notes where added_by=%s',[added_by])
        data=cursor.fetchall()
        print(data)
...
def forgetpassword():
    if session.get('email'):
        ...
        if request.method=='POST':
            email=request.form['email']
            cursor=mydb.cursor(buffered=True)
            cursor.execute('select count(email) from student1 where email=%s',[email])
    ...
    try:
        ...
        if request.method=='POST':
            ...
            if npassword==cpassword:
                cursor=mydb.cursor(buffered=True)
                cursor.execute('update student1 as set password=%s where email=%s',[npassword,email])
                mydb.commit()
                cursor.close()
